...
namespace ClassLibrary2361
{
    ...
    public class FieldSet :GenerationSet
    {
        ...
        private static Field FillGenePool(BleepyBloop[] Olds, List<Instruction[]> Clones, List<Instruction> GenePool)
        {
            ...
            double Mean = Olds.Sum(x => x.ObjectiveFunction())/ 32;
            foreach(BleepyBloop b in Olds)
            {
                int NumberOfInstructionsToPassOn;
                if (b.ObjectiveFunction() >= Mean)
                {
                    Clones.Add(b.Genes);
                    NumberOfInstructionsToPassOn = (int)Math.Round(BleepyBloop.MemSize * ((b.ObjectiveFunction() / Mean) - 1));
                }
                else
                {
                    NumberOfInstructionsToPassOn = (int)Math.Round(BleepyBloop.MemSize * ((b.ObjectiveFunction() / Mean)));
                }
